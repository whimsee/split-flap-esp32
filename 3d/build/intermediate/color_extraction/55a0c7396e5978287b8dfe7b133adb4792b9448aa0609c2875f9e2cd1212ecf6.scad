...
module spool_struts() {
    for (i=[0:3]) {
        angle = 90*i;
        // color_extractor([i < 2 ? 0 : 1, i == 0 || i == 2 ? 0 : 1, 0])
         color_extractor(i % 2 == 0 ? assembly_colors[1] : assembly_colors[2])
...
module alignment_bar() {
     color_extractor(assembly_colors[0])
...
module split_flap_3d(front_flap_index, include_connector, include_front_panel=true) {
    ...
    module positioned_enclosure() {
        if (render_enclosure == 2) {
             color_extractor(assembly_colors[0])
                positioned_front();
             color_extractor(assembly_colors[1])
                positioned_left();
             color_extractor(assembly_colors[1])
                positioned_right();
             color_extractor(assembly_colors[2])
                positioned_top();
             color_extractor(assembly_colors[2])
                positioned_bottom();
            ...
            if (include_connector) {
                 color_extractor(assembly_colors[3])
    ...
    if(render_spool) {
        translate([(spool_width_clearance - spool_width + spool_width_slop) / 2 + thickness, 0, 0]) {
            ...
            translate([spool_width - thickness + 5*spool_horizontal_explosion, 0, 0]) {
                rotate([0, 90, 0]) {
                     color_extractor(assembly_color)
